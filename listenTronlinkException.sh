...
do 
  echo $deviceName
  nohup /Users/tron/Library/Android/sdk/platform-tools/adb -s $deviceName logcat | grep " E " -A 35 | grep  'FATAL|Crash|AndroidRuntime' -A 35 | grep -A 35 'wallet.tronlink.global'  > exception_$deviceName.log 2>&1 &
  echo "adb nohup"
done
